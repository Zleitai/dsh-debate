 ...
 */
function mount(registerTools: boolean): unknown[] {
...
test('the host web-plane mount (registerTools: false) registers nothing', () => {
  ...
  // to every session and collide with the agent-plane registration. It is the
  // explicit config switch — not a ctx.agent probe — that disables registration:
  // a preset's standing mount also has no Agent at apply time.
  assert.deepEqual(mount(false), []);
});
